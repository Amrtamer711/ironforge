...
const ModuleRegistry = {
  ...
  setupMobileToggle() {
    const logoBtn = document.getElementById('mobileSidebarLogoToggle');
    ...
    const toggleMobile = () => {
      if (!isMobile()) return;
      document.body.classList.contains('mobile-sidebar-open') ? closeMobile() : openMobile();
    };
    ...
    sidebar.addEventListener('click', (e) => {
      if (!isMobile()) return;
      if (e.target.closest('.sidebar-nav-item')) closeMobile();
    });
