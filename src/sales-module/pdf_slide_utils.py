...
import config
from generators.pdf import _CONVERT_SEMAPHORE, convert_pptx_to_pdf
...
async def extract_first_and_last_slide_as_pdfs(file_path: str) -> tuple[str, str]:
    ...
    async with _CONVERT_SEMAPHORE:
        ...
        if file_path.lower().endswith('.pdf'):
            logger.info("[EXTRACT_SLIDES] ✅ Input is already a PDF, using directly (no conversion needed)")
            logger.info(f"[EXTRACT_SLIDES] PDF path: {file_path}")
            full_pdf = file_path
            should_delete_full_pdf = False
        else:
            # Convert PowerPoint to PDF with HIGH QUALITY
            logger.info("[EXTRACT_SLIDES] 🔄 Converting PowerPoint to PDF with HIGH QUALITY")
