...
class SQLiteBackend(DatabaseBackend):
    ...
    def create_role(
        ...
        try:
            ...
            cursor.execute(
                ...
                """,
                (name, description, 1 if is_system else 0, created_at),
            )
            conn.execute("COMMIT")
            ...
            row = cursor.fetchone()
            role_id = str(row[0]) if row else None

            logger.info(f"[DB] Created/updated role: {name} (id={role_id})")
            return role_id
        except Exception as e:
            conn.execute("ROLLBACK")
    ...
    def update_role(
        ...
        try:
            ...
            conn.execute("ROLLBACK")
            logger.error(f"[DB] Error updating role: {e}")
            return False
        finally:
            conn.close()
    ...
    def delete_role(self, role_id: str) -> bool:
        ...
        try:
            ...
            conn.execute("COMMIT")
            logger.info(f"[DB] Deleted role: {role_id}")
            return True
        except Exception as e:
            conn.execute("ROLLBACK")
            logger.error(f"[DB] Error deleting role: {e}")
            return False
        finally:
            conn.close()
    ...
    def assign_role_permission(
        ...
        try:
            ...
            row = cursor.fetchone()
            if not row:
                logger.warning(f"[DB] Permission not found: {permission_name}")
                return False
            perm_id = row[0]
            ...
            conn.execute(
                ...
                """,
                (role_id, perm_id, assigned_at),
            )
            conn.execute("COMMIT")
            return True
        except Exception as e:
            conn.execute("ROLLBACK")
            logger.error(f"[DB] Error assigning role permission: {e}")
            return False
        finally:
            conn.close()
    ...
    def set_role_permissions(
        ...
        try:
            ...
            conn.execute("ROLLBACK")
            logger.error(f"[DB] Error setting role permissions: {e}")
            return False
        finally:
            conn.close()
    ...
    def get_user_roles(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            cursor.execute(
                """
                SELECT r.*, ur.granted_at, ur.granted_by, ur.expires_at
                FROM roles r
    ...
    def assign_user_role(
        ...
        try:
            ...
            conn.execute("COMMIT")
            logger.info(f"[DB] Assigned role {role_id} to user {user_id}")
            return True
        except Exception as e:
            conn.execute("ROLLBACK")
            logger.error(f"[DB] Error assigning user role: {e}")
            return False
        finally:
            conn.close()
    ...
    def revoke_user_role(self, user_id: str, role_name: str) -> bool:
        ...
        try:
            ...
            row = cursor.fetchone()
            if not row:
                logger.warning(f"[DB] Role not found: {role_name}")
                return False
            role_id = row[0]
            ...
            conn.execute("COMMIT")
            logger.info(f"[DB] Revoked role {role_name} from user {user_id}")
            return True
        except Exception as e:
            conn.execute("ROLLBACK")
            logger.error(f"[DB] Error revoking user role: {e}")
            return False
        finally:
            conn.close()
